    ...
    public function createOrder()
    {
        ...
        if ($this->getSession()->getOrder()->getId()) {
            ...
            if ($used_amount > 0) {
                 ...
                 $orderData = array(
                    'order_from'         => $post['order_from'],
                    'order_trench'         => $post['order_trench'],
                    'order_admin'         => $post['order_admin'],
                    'base_grand_total'   => $grandTotal,
                    'grand_total'        => $grandTotal,
                    'credit_amount'      => $creditAmount,
                    'base_credit_amount' => $creditAmount,
                    'credit_qty'         => $used_amount
                );
         //       $address=  $this->getBillingAddress();
            //    $address->addTotalAmount('',$creditAmount);
             //   $address->addBaseTotalAmount('',$creditAmount);
             }else{
                $orderData = array(
                    'order_from' => $post['order_from'],
                    'order_trench'         => $post['order_trench'],
        ...
        if ($this->getSession()->getOrder()->getId()) {
            ...
            $order->save();
            if($creditAmount) {
                $creditsModel = Mage::getModel('d1m_credits/credits');
                $userId = $order->getCustomerId();
                $creditInfo = $creditsModel->load($userId, 'customer_id');
                if($creditInfo->getCreditAmount()) {
                    $diffCredit = $creditInfo->getCreditAmount() - $creditAmount;
                    $creditsModel->setCreditAmount($diffCredit)->save();
                }
            }
        }
