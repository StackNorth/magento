...
class Mage_Adminhtml_Sales_Order_CreateController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _processActionData($action = null)
    {
        ...
        } elseif (isset($data) && isset($data['use_credit_amount'])) {
            $used_amount = intval($data['use_credit_amount']);
            $customerId = $this->getRequest()->getPost('customer_id');
            $allCredits = Mage::helper('d1m_credits')->getCreditAmountByCustomerId($customerId);
            if ($allCredits < $used_amount) {
                $this->_getSession()->addError($this->__('用户课点余额为:' . $allCredits . ',不够此次使用.'));
            } else {

                Mage::helper('settleaccount/event')->setPrepareUsedCredit($used_amount);
                $credit_apply = $used_amount * Mage::helper('d1m_credits')->getFeeByCredit();
                $billingAddress = $this->_getOrderCreateModel()->getBillingAddress();//->getId();
                  $total = $billingAddress->getGrandTotal();
                $grandTotal = $total - $credit_apply;
                $creditAmount = $used_amount * -1;
                $addressMoney = array(
                    'discount_amount' => $creditAmount,
                    'base_discount_amount' => $creditAmount,
                    'grand_total' => $grandTotal,
                    'total_paid' => $grandTotal,
                    'base_grand_total' => $grandTotal
                );

                $billingAddress->setData($addressMoney)->setAddressType(Mage_Sales_Model_Quote_Address::TYPE_BILLING);
                $this->_getSession()->addSuccess($this->__('用户成功使用' . $used_amount . '课点.'));
            }
    ...
    public function loadBlockAction()
    {
        ...
        // print_r($request->getPost('customer_id'));die;
        try {
            $this->_initSession()
                ->_processData();
