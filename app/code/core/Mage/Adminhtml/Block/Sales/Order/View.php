    ...
    public function __construct()
    {
       ...
       // if ($this->_isAllowedAction('cancel') && $order->canCancel()) {
        if ($order->getState()=='complete') {
            ...
            $cancelUrl=$this->getUrl('*/report/cancelOrder');
            $this->_addButton('order_cancel', array(
                'label'     => Mage::helper('sales')->__('Cancel'),
                'onclick'   => 'deleteConfirm(\''.$message.'\', \'' . $cancelUrl . '\')',
            ));
            $message = Mage::helper('sales')->__('Are you sure you want to reversed this order?');
            $reversedUrl=$this->getUrl('*/report/reversedOrder');
            $this->_addButton('paypal_reversed', array(
                'label'     => Mage::helper('sales')->__('退款'),
                'onclick'   => 'deleteConfirm(\''.$message.'\', \'' . $reversedUrl . '\')',
            ));
        }
