 ...
 */
class D1m_CouponRule_Model_Payment extends Mage_Payment_Model_Method_Abstract
{
    protected $_code  = 'couponRule_payment';
    protected $_formBlockType = 'couponRule/form';
    ...
    protected $_canUseInternal          = true; // 在后台可用
    ...
    public function getCreditOrderPlaceRedirectUrl()
    {
        return Mage::getUrl('alipay/payment/creditredirect');
    }
     ...
     *  @return	  string URL
     */
    protected function getCreditOrderReturnURL()
    {
        return Mage::getUrl('alipay/payment/creditsuccess', array('_secure' => true));
    }
     ...
     *  @return	  string URL
     */
    protected function getCreditOrderErrorURL()
    {
        return Mage::getUrl('alipay/payment/crediterror', array('_secure' => true));
    }
     ...
     *  @return	  string URL
     */
    protected function getCreditOrderNotifyURL()
    {
        return Mage::getUrl('alipay/payment/creditnotify', array('_secure' => true));
    }
    ...
    public function getStandardCheckoutFormFields()
    {
        ...
        if($this->getConfigData('service_type')=="create_forex_trade")
        {
            ...
            $parameter = array('service'           => $this->getConfigData('service_type'),
                ...
                'subject'           => 'Order_'.$order->getRealOrderId(),
        ...
        else
        {
            ...
            $parameter = array('service'           => $this->getConfigData('service_type'),
                ...
                'notify_url'        => $this->getNotifyURL(),
                '_input_charset'    => 'utf-8',
                'subject'           =>'Order_'.$prefix_order.'_'. $order->getRealOrderId(),
                'body'              => $order->getRealOrderId(),
                'out_trade_no'      => $prefix_order.$order->getRealOrderId(), // order ID
...
//		$arg = "";
    ...
    /**
     *  Return CreditOrder Standard Checkout Form Fields for request to Alipay
     *
     *  @return	  array Array of hidden form fields
     */
    public function getCreditOrderStandardCheckoutFormFields()
    {
//        $session = Mage::getSingleton('checkout/session');

        $creditorder = $this->getCreditOrder();
        if (!($creditorder instanceof D1m_Credits_Model_Order)) {
            Mage::throwException($this->_getHelper()->__('Cannot retrieve order object'));
        }


        $converted_final_price = $creditorder->getGrandTotal();

        if($this->getConfigData('service_type')=="create_forex_trade"){

            $parameter = array('service'           => $this->getConfigData('service_type'),
                'partner'           => $this->getConfigData('partner_id'),
                'return_url'        => $this->getCreditOrderReturnURL(),
                'notify_url'        => $this->getCreditOrderNotifyURL(),
                '_input_charset'    => 'utf-8',
                'subject'           =>$creditorder->getRealOrderId(),
                'body'              => $creditorder->getRealOrderId(),
                'out_trade_no'      => $creditorder->getRealOrderId(), // order ID
                'total_fee'             => sprintf('%.2f', $converted_final_price) ,
                'currency'      => 'USD'
            );
        }else{
            ...
            if(Mage::app()->getStore()->getCurrentCurrencyCode() !=$toCur){
                if(Mage::app()->getStore()->getBaseCurrencyCode()!=$toCur){
                    ...
                    $rate=Mage::getModel('directory/currency')->load($toCur)->getAnyRate($fromCur);
                    $converted_final_price= $creditorder->getGrandTotal()/$rate;
            ...
            $parameter = array('service'           => $this->getConfigData('service_type'),
                ...
                'notify_url'        => $this->getCreditOrderNotifyURL(),
                '_input_charset'    => 'utf-8',
                'subject'           => 'Order_'.$prefix_order.'_'.$creditorder->getRealOrderId(),
                'body'              => $creditorder->getRealOrderId(),
                'out_trade_no'      => $prefix_order.$creditorder->getRealOrderId(), // order ID 前面加c与课程订单区分开来
                'logistics_fee'     => '0.00', //because magento has shipping system, it has included shipping price
                'logistics_payment' => 'BUYER_PAY',  //always
                'logistics_type'    => 'EXPRESS', //Only three shipping method:POST,EMS,EXPRESS
                'price'             => sprintf('%.2f', $converted_final_price) ,
