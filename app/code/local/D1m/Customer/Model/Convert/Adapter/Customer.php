<?php
class D1m_Customer_Model_Convert_Adapter_Customer
    extends Mage_Customer_Model_Convert_Adapter_Customer
{
    ...
    public function saveRow($importData)
    {
        ...
        $doLoadByAcct = false;
        if (!empty($importData['account_number'])) {
            $doLoadByAcct = true;
        }
        ...
        if ($doLoadByAcct) {
            $customer->setWebsiteId($website->getId())
                ->loadByAccountId($importData['account_number']);
        }
        else {
            $customer->setWebsiteId($website->getId())
        ...
        foreach ($importData as $field => $value) {
            $isNoForcedUpdate = false;
            if (substr($value,0,3)=="___")
            {
                $isNoForcedUpdate = true;
            ...
            // Added a "no update" value for
            if (!($customer->getId() && $isNoForcedUpdate))
        ...
        $importBillingAddress = $importShippingAddress = false;
...
//         }
		...
		foreach ($this->_addressFields as $field) {
			if (isset($importData['billing_'.$field]) && !empty($importData['billing_'.$field])) {
				$importBillingAddress = true;
			}
			if (isset($importData['shipping_'.$field]) && !empty($importData['shipping_'.$field])) {
        ...
        if ($importBillingAddress) {
			...
			$importData['billing_country'] = $this->getCountryId($importData['billing_country']);
        ...
        if ($importShippingAddress) {
			...
			$importData['shipping_country'] = $this->getCountryId($importData['shipping_country']);
