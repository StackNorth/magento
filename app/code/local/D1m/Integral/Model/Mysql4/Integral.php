<?php
class D1m_Integral_Model_Mysql4_Integral extends Mage_Core_Model_Mysql4_Abstract
{
	
    protected function _construct()
    {       
        $this->_init('d1m_integral/integral', 'id');
    } 
    ...
    protected function _beforeSave(Mage_Core_Model_Abstract $object)
    {
        ...
        if($creditsId = $object->getId())
        {
            ...
            if($add || $subtract)
            {
                $history = Mage::getModel('d1m_integral/history'); 
                $history->credit_id = $object->getId();
                $history->add = $add;
    ...
    public function _afterSave(Mage_Core_Model_Abstract $object)
    {
    	...
    	if($object->_historyRec && $object->_historyRec instanceof D1m_Integral_Model_History )
    	{
