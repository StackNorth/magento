...
class Mage_SalesRule_Model_Resource_Rule_Collection extends Mage_Rule_Model_Resource_Rule_Collection_Abstract
{
     ...
     * @param null $now
     * @return $this|Mage_SalesRule_Model_Resource_Rule_Collection
     */
    public function setValidationFilter($websiteId, $customerGroupId, $couponCode = '', $now = null)
    {
        if (!$this->getFlag('validation_filter')) {
            ...
            if (strlen($couponCode)) {
                ...
                $select->where('(main_table.coupon_type = ? ', Mage_SalesRule_Model_Rule::COUPON_TYPE_NO_COUPON)
                    ...
                    //add auto generate coupon
                    ->orWhere('main_table.coupon_type = ? ' .
                    'AND rule_coupons.type = '.D1m_CouponRule_Model_Rule::COUPON_TYPE_AUTO_GENERATE_WITH_CUSTOMER , D1m_CouponRule_Model_Rule::COUPON_TYPE_AUTO_GENERATE_WITH_CUSTOMER)
                    //add customer event
                    ->orWhere('main_table.coupon_type = ? ' .
                    'AND rule_coupons.type = '.D1m_CouponRule_Model_Rule::COUPON_TYPE_AUTO_GENERATE_FOR_EVENT , D1m_CouponRule_Model_Rule::COUPON_TYPE_AUTO_GENERATE_FOR_EVENT)
        ...
        Mage::log((string)$this->getSelect(),null,'sql.log');
        return $this;
    }
