...
D1.url.rootDomain = 'eskin.com.cn';
D1.url.skinPath = '/skin/frontend/d1m/happyboxmobile/';

D1.url.path = location.pathname;
D1.url.isHome = (D1.url.path === '/' || D1.str.ifStartsWith(D1.url.path, '/static/html/home/'));
...
D1.sns = D1.sns || {};
D1.sns.link = {
	weibo: "http://v.t.sina.com.cn/share/share.php?url=[[[url]]]&title=[[[text]]]&appkey=",
	...
	tengxun: "http://v.t.qq.com/share/share.php?url=[[[url]]]&title=[[[text]]]&appkey=",
	end: null
};
D1.sns.getShareLink = function (site, url, text) {
	var link = '';
	if (site && url && text) {
		var temp = D1.sns.link[site];
		if (temp) {
			link = D1.str.replaceVar(temp, '[[[url]]]', encodeURIComponent(url));
			link = D1.str.replaceVar(link, '[[[text]]]', encodeURIComponent(text));
		}
	}
	return link || false;
};
